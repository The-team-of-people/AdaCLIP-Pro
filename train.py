...
def setup_dataloaders(cfg, device, train_annot, val_annot):
    ...
    # ---- 分布式环境下才用DistributedSampler，否则为None ----
    sampler = DistributedSampler(train_dataset) if is_distributed_env() else None
...
def get_embeddings(val_loader, model, cfg):
    with torch.no_grad():
        ...
        for minibatch in val_loader:
            output = model(minibatch["text_input_ids"], minibatch["clip_inputs"], minibatch["policy_inputs"],
                           return_embds=True)
...
def train(cfg):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu", getattr(cfg, "local_rank", 0))
    ...
    for epoch in range(cfg.num_epochs):
        ...
        # 分布式环境下才有sampler
        if is_distributed_env() and hasattr(train_loader, "sampler") and hasattr(train_loader.sampler, "set_epoch"):
        ...
        for step, minibatch in enumerate(train_loader):
            ...
            sim_matrix, actions = model(minibatch["text_input_ids"], minibatch["clip_inputs"],
                                        minibatch["policy_inputs"], tau=tau, gather=True)
            ...
            if (step + 1) % cfg.gradient_accumulation_steps == 0:
                ...
                if global_step % cfg.n_display == 0:
                    ...
                    lr = '-'.join([str('%.9f' % itm) for itm in sorted(
                        list(set(optimizer.get_lr())))]) if cfg.optim == "bertadam" else cfg.learning_rate
